...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!description || !category || !userid || sum === undefined) {
            return res.status(400).json({ error: 'Missing required fields', required: ['description', 'category', 'userid', 'sum'] });
        }
        ...
        // STEP 4: Create cost data with legitmate values
        const costData = {
            description,
            category,
            userid: Number(userid),
            sum: Number(sum),
            created_at: created_at || new Date() // Use provided date or current date if not provided
        };
